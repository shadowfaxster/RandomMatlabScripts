...
% dimension stores a 2D array in the form described for each position. 
% 
% For invalid values (-.--), the RT time returned is 999. 
%
...
% ----------------------------------------
% 
% TODO: catch if 2 consecutive files have been deleted. At the moment it  works
...
% once and not once per position.
    ...
    for pos=1:no_of_positions    % Changes every time a new file is loaded (a.i. new position)
        if filenumber<10
        ...
        if exist(filename, 'file')       
            ...
            % file doesn't exist, so maybe you've deleted that 
	    % particular file, but the next one is still good
